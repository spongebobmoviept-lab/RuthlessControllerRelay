/*
 * ============================================================================
 *  DO NOT USE THIS FILE. IT IS KNOWN BROKEN. KEPT FOR REFERENCE ONLY.
 * ============================================================================
 *
 * This is a real, byte-for-byte snapshot of an actual failed experiment --
 * not a simplified example, the genuine source tree from the moment this
 * attempt was tried. It is a full, otherwise-working copy of the relay with
 * ONE additional change: re-enabling Bluetooth PlayStation-controller rumble/
 * LED output (grep for "EXPERIMENTAL RE-ENABLE 2026-09-09" to find every
 * spot that differs from the real, shipped source in src/hotas_relay.c).
 *
 * What it tried: sending rumble/LED output to a DualShock 4 or DualSense
 * over Bluetooth using WriteFile() on the SAME handle already being used to
 * continuously read input -- the one specific combination that hadn't been
 * isolated and tested on its own yet at the time.
 *
 * What happened on real hardware: the Bluetooth connection destabilized
 * within about 10 seconds -- dozens of rapid connect/disconnect/reconnect
 * cycles -- on TWO different Bluetooth adapters from two different chipset
 * vendors. Reverted immediately per this project's own standing rule
 * (revert on any sign of instability). A real power-cycle of the controller
 * was needed afterward to fully clear the resulting bad Bluetooth link
 * state -- a plain code revert alone did not undo it.
 *
 * Full writeup, including the two research passes that led here and the
 * final conclusion (this is a genuine, still-unsolved, community-wide
 * problem, not specific to this project): see
 * ../docs/KNOWN_LIMITATIONS.md#bluetooth-playstation-controller-rumble-and-led-output
 * and ../docs/DEVELOPMENT_JOURNEY.md.
 *
 * This file will not build correctly against the current third_party/
 * headers used elsewhere in this repo without adjustment -- it's a snapshot
 * from a specific point in the project's history, not maintained, not kept
 * in sync with the real source, and not intended to compile as-is. It's
 * here so nobody has to reconstruct or re-attempt this exact combination
 * from scratch to find out it doesn't work.
 * ============================================================================
 */
...
static void set_color(WORD attr) { SetConsoleTextAttribute(g_console, attr); }
...
static void draw_button(const char *label, int pressed, const char *remap_label) {
    ...
    set_color(pressed ? CLR_GREEN_BR : CLR_GRAY_DIM);
...
#define DASHBOARD_ROWS_NEEDED 34
...
static void send_rumble_led_to_real_ps_controller(BYTE motorLeft, BYTE motorRight, BYTE r, BYTE g, BYTE b) {
    ...
    /* REAL BUG found and fixed 2026-09-08, on real hardware, after the
       ...
       HidD_SetOutputReport too, in BOTH cases tested with a SEPARATE
       output handle from the continuous-read one.
       EXPERIMENTAL RE-ENABLE 2026-09-09, explicit user request: the ONE
       combination never actually isolated on its own -- the SAME handle
       as the continuous read (not a second one), using HidD_SetOutputReport
       specifically (not WriteFile, which is what the same-handle attempt
       above actually used) -- see setup_ds4_raw_hid()/
       setup_dualsense_raw_hid()'s own comments for exactly what's
       different this time. Genuinely untested; revert this single early-
       return (and the matching handle-assignment blocks in both raw-HID
       setup functions) immediately on any sign of BT input instability,
       same as every other regression tonight -- reliable input matters
       far more than a nice-to-have lightbar/rumble. Wired PS controllers
       (this session's other change) DO now share a read+write handle too,
       successfully -- USB and Bluetooth are different transports/driver
       stacks though, so that success doesn't predict this one either way. */
    BYTE report[78] = {0};
    DWORD len;
        ...
        if (HidD_GetAttributes(h, &attrs) && attrs.VendorID == 0x054C && is_ds4_product_id(attrs.ProductID)) { /* Sony, and specifically a DS4 */
            ...
            if (gotCaps && caps.InputReportByteLength > 64 && caps.InputReportByteLength <= 1024) {
                ...
                /* REAL BUG found and fixed 2026-09-08, on real hardware, after
                   ...
                   EXPERIMENTAL RE-ENABLE 2026-09-09, explicit user request
                   after being told this exact combination already failed
                   once tonight: the "same handle" attempt described above
                   used WriteFile() for output; this specific pairing --
                   same handle, but HidD_SetOutputReport() instead (see
                   send_rumble_led_to_real_ps_controller()'s own updated
                   comment) -- was never actually isolated on its own, only
                   tried together with a SEPARATE handle. Genuinely
                   untested, going in with real risk to BT input's hard-won
                   reliability -- revert this one block (and the matching
                   one in setup_dualsense_raw_hid(), and the early-return in
                   send_rumble_led_to_real_ps_controller()) immediately on
                   ANY sign of instability, same as every other regression
                   tonight. */
                g_ps_output_handle = h;
                g_ps_output_is_bt = 1;
                g_ps_output_is_dualsense = 0;
                g_ps_output_owns_handle = 0;
                g_ps_output_shares_input_handle = 1;
        ...
        if (HidD_GetAttributes(h, &attrs) && attrs.VendorID == 0x054C && attrs.ProductID == 0x0CE6) {
            ...
            if (HidD_GetPreparsedData(h, &preparsed) && HidP_GetCaps(preparsed, &caps) == HIDP_STATUS_SUCCESS &&
                ...
                /* REAL BUG found and fixed 2026-09-08, on real hardware, after
                   ...
                   EXPERIMENTAL RE-ENABLE 2026-09-09 -- see the matching,
                   more detailed comment in setup_ds4_raw_hid() for exactly
                   what's different this time and why it might not fail the
                   same way. Revert immediately on any sign of instability. */
                g_ps_output_handle = h;
                g_ps_output_is_bt = 1;
                g_ps_output_is_dualsense = 1;
                g_ps_output_owns_handle = 0;
                g_ps_output_shares_input_handle = 1;
